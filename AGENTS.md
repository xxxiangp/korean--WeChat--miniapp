...
韩语背单词微信小程序项目。包含产品设计文档、词汇数据、测验数据、音频素材及工具脚本。当前阶段：**开发中**。通用三级词书已接入；TOPIK 初级/中级/高级数据已交付，六词书界面、跨词书进度共享与 TOPIK 音频仍待接入。
...
│   │   ├── topik_intermediate.json # TOPIK 中级备考词书（3715词）
│   │   ├── topik_intermediate_quiz.json # TOPIK 中级四选一干扰项数据
│   │   ├── topik_advanced.json  # TOPIK 高级备考词书（1008词）
│   │   ├── topik_advanced_quiz.json # TOPIK 高级四选一干扰项数据
│   │   ├── topik_ii_*_audit.json # TOPIK II 来源、词典元数据与逐词审计快照
...
│   │   ├── TOPIK中级词汇_最终交付版.docx # TOPIK 中级最终交付 Word（3715词）
│   │   ├── TOPIK高级词汇_最终交付版.docx # TOPIK 高级最终交付 Word（1008词）
...
│       ├── extract_nikl_learning_vocab.ps1 # 提取并校验国立国语院官方分级词表
│       ├── build_topik_ii_source_pools.py # 构建 TOPIK 中高级候选池与来源审计
│       ├── fetch_topik_ii_krdict_metadata.py # 抓取韩国语基础词典官方元数据
│       ├── generate_topik_ii_wordbooks.py # 生成 TOPIK 中高级 JSON/Quiz/Word 与元数据审计
...
通用三级词书：2942 词（初级 1282 / 中级 901 / 高级 759）。

TOPIK 独立词书：6509 词（初级 1786 / 中级 3715 / 高级 1008），三本之间词形零重复。六个词书文件共 9451 条，按韩语词形去重后为 7619 词；TOPIK 与通用三级词书重合 1832 词。
...
12. **TOPIK II 交付口径**：TOPIK 官方不发布固定逐词考纲；中高级文件只能称为“交叉审计形成的备考词书”，不能称为“官方词表”。最终释义优先使用韩国国立国语院《韩国语基础词典》中文版；公开词表误释、人工合并和 g2pk2 发音回退必须保留审计记录。
...
- TOPIK 三本数据已同步到 `miniprogram/data`，但当前 TypeScript `Level`、选词书页、共享进度、云同步与音频尚未扩展；不能把“数据存在”表述成“已接入可用”
- 发音标注务必检查经音化等音变规则
